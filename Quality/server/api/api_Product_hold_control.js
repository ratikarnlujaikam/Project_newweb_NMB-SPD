...
router.get(
 ...
 WITH [Record_QAPrint] AS (
		  ...
		  WHERE CONVERT(DATE, [Record_QAPrint].DateTime) >= DATEADD(MONTH, -12, GETDATE())
      ...
      ,[tbQA] AS (
          ...
          inner JOIN [QAInspection].[dbo].[tbQANumber] ON s.[QANumber] = [tbQANumber].[QANumber]
          WHERE [Vis_Round] = (SELECT MAX([Vis_Round]) FROM [QAInspection].[dbo].[tbVisualInspection] WHERE [QANumber] = s.[QANumber])
         and CONVERT(DATE, [tbQANumber].[Time]) >= DATEADD(MONTH, -12, GETDATE())
		 GROUP BY
              [Date],
              [Model_group],
              s.[ModelNumber],
