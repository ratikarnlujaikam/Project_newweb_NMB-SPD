...
router.get("/LARMonth/:product_type/:Model/:insType/:year/:Line",
  async (req, res) => {
    try {
      var result = [[]];
      const { year,Model, insType,Line,product_type} = req.params;
  ...
  --Case 1: All are "**ALL**"
			  ...
			  ,AA as (select month([InspectionDate]) as Date, [InspectionResult],CAST(count([InspectionResult]) AS FLOAT) AS RESULT_QTY
