...
router.get(
  ...
  async (req, res) => {
   ...
   FROM [QAInspection].[dbo].[tbVisualInspection] 
	...
	t2 AS (  SELECT [QANumber],[ModelGroup],[ModelNumber],[Line_No] as Line_No ,Remark,sum([MOQTY]) as MOQTY
   FROM [QAInspection].[dbo].[tbQANumber] 

   GROUP BY [QANumber],[Line_No],Remark,[ModelGroup],[ModelNumber])

   SELECT [InspectionType] as Type,[InspectionDate] as Date,[InspectionShift]as Shift,[ModelNumber]as Model_NO,
   [ModelGroup] as Model_Group ,[Model_Name]as Model_Name,'L'+[Line_No] as Line_No,T1.[QANumber],[MOQTY]as QA_QTY,[SamplingLevel],[SamplingQTY],[Vis_Round],[InspectionResult],QTY as Reject_QTY,Remark
   ...
   ,T1.[QANumber],[MOQTY],[SamplingLevel],[SamplingQTY],[Vis_Round],[InspectionResult],Remark,QTY
